...
private:
    ...
    void insertWord(int line, int parse, std::string input01) {
            std::ifstream in(text);
            if (!in.is_open()) { throw std::exception("Can't find the file"); }
            std::string input;
            in.clear();
            std::getline(in, input);
            in.close();
            std::ofstream out(text);
            if (!out.is_open()) { throw std::exception("Can't find the file"); }
...
public:
    ...
    bool insertFunc(char* cmd) {
        ...
        std::stringstream l, p;
            ...
            if (strtok(NULL, ",") != NULL)
                throw std::exception("! 입력 형식이 잘못되었습니다 : i(int line, int parse, string input)");
    ...
    bool changeFunc(char* cmd) {
        ...
        if (input01.size() >= 75 || input02.size() >= 75)
            throw std::exception("! string은 75자 미만 입출력이 가능합니다.");
...
int main() {
    ...
    try {
        ...
        switch (cmd[0]) {
        case 'n':
            if (e->next()) std::cout << "-----------------------------------------------------------------------------------------" << std::endl;
            e->printFile();
            break;
        case 'p':
            if (e->prev()) std::cout << "-----------------------------------------------------------------------------------------" << std::endl;
            e->printFile();
            break;
        case 'i':
            if (!e->insertFunc(cmd)) {
        ...
        std::cout << "! 형식에 맞지 않는 입력값입니다. 다시 입력하세요." << std::endl;
        std::cout << "-----------------------------------------------------------------------------------------" << std::endl;
        e->printFile();
    }
    } while (cmd[0] != 't');
