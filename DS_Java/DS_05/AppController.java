...
public class AppController {
    ...
    public AppController(){ this.setList(new LinkedList<Student>()); }

    //<GETTER SETTER START>
    public LinkedList<Student> list(){ return this._list; }
    private void setList(LinkedList<Student> newList){ this._list = newList; }
    //<GETTER SETTER END>

    /*
    private ArrayList<Student> _list;

    public AppController(){ this.setList(new ArrayList<Student>(LIST_CAPACITY)); }

    //<GETTER SETTER START>
    public ArrayList<Student> list(){ return this._list; }
    private void setList(ArrayList<Student> newList){ this._list = newList; }
    //<GETTER SETTER END>
*/
    ...
    private MainMenu selectMenu(){
        ...
        this.showMenu();
        try{
            int selectedMenuNumber = AppView.inputInteger();//숫자를 예외처리함
            MainMenu selectedMenuValue = MainMenu.value(selectedMenuNumber);
            if(selectedMenuValue == MainMenu.Error){
                AppView.outputLine("!오류 : 작업 선택이 잘못되었습니다. (잘돗된 메뉴 번호 : " +selectedMenuNumber+ ")");
            }
    ...
    private void doesContain() {
        ...
        if (this.list().isEmpty())
            AppView.outputLine("! 리스트가 비어 있어서 작업을 진행할 수 없습니다.");
        else {
            int score = this.inputScore();
            if (this.list().doesContain(new Student(score)))
                AppView.outputLine("! 입력된 점수(" + score + ")을 가진 학생이 존재합니다.");
            else
                AppView.outputLine("! 입력된 점수(" + score + ")을 가진 학생이 존재하지 않습니다.");
        }
    }
    ...
    private void elementAt(){
        ...
        if(this.list().isEmpty())
            ...
            if(order < 0 || order >= this.list().size())
                AppView.outputLine("! 입력된 순서[" +  order + "]가 정상범위 [ 0.. "+ (this.list().size()-1) + "]에 있지 않습니다.");
            else{
                Student student = this.list().elementAt(order);
                if(student == null)
                    AppView.outputLine("! 입력된 순서[" +  order + "]에 학생이 존재하지 않습니다." );
                else
                    AppView.outputLine("! 입력된 순서[" + order + "]에 점수 (" + student.score() + ")를 받은 학생이 존재합니다." );
            }
        }
    }
    ...
    private void first(){
        ...
        if(this.list().isEmpty())
            ...
            if(student == null)
                AppView.outputLine("! [맨 앞]에 입력된 학생을 불러오는 작업을 실패하였습니다." );
            else
                AppView.outputLine("! [맨 앞]에 입력된 점수(" + student.score() + ")의 학생을 불러오는 작업을 성공하였습니다." );
        }
    }
    ...
    private void last(){
        ...
        if(this.list().isEmpty())
            ...
            if(student == null)
                AppView.outputLine("! [맨 뒤]에 입력된 학생을 불러오는 작업을 실패하였습니다." );
            else
                AppView.outputLine("! [맨 뒤]에 입력된 점수(" + student.score() + ")의 학생을 불러오는 작업을 성공하였습니다." );
        }
    }
    ...
    private void orderOf() {
        ...
        if (this.list().isEmpty())
            ...
            if (order < 0 || order > this.list().size())
                AppView.outputLine("! 입력된 점수(" + score + ")를 가지는 학생이 존재하지 않습니다.");
            else
                AppView.outputLine("! 해당 순서[" + order + "]에 입력된 점수(" + score + ")를 가지는 학생이 존재합니다.");
        }
    }
    ...
    private void addTo(){
        ...
        if(this.list().isFull())
            ...
            int order = this.inputOrder();
            if(order < 0 || order > this.list().size())
                AppView.outputLine("! 입력된 순서[" +  order + "]가 정상범위 [ 0.. "+ this.list().size() + "]에 있지 않습니다.");
            else{
                int score = this.inputScore();
    ...
    private void addToFirst(){
        ...
        if(this.list().isFull())
            ...
            if(this.list().addToFirst(new Student(score)))
                AppView.outputLine("! [맨 앞]에 입력된 점수(" + score + ")의 학생을 삽입하는 작업을 성공하였습니다." );
            else
                AppView.outputLine("! [맨 앞]에 입력된 점수(" + score + ")의 학생을 삽입하는 작업을 실패하였습니다." );
            }
    }
    ...
    private void addToLast(){
        ...
        if(this.list().isFull())
            ...
            if(this.list().addToLast(new Student(score)))
                AppView.outputLine("! [맨 뒤]에 입력된 점수(" + score + ")의 학생을 삽입하는 작업을 성공하였습니다." );
            else
                AppView.outputLine("! [맨 뒤]에 입력된 점수(" + score + ")의 학생을 삽입하는 작업을 실패하였습니다." );
            }
    }
    ...
    private void add(){
        ...
        if(this.list().isFull())
            ...
            if(this.list().add(new Student(score)))
                AppView.outputLine("! 입력된 점수(" + score + ")의 학생을 삽입하는 작업을 성공하였습니다." );
            else
                AppView.outputLine("! 입력된 점수(" + score + ")의 학생을 삽입하는 작업을 실패하였습니다." );
        }
    }
        ...
        if(this.list().isEmpty())
            ...
            if(order < 0 || order >= this.list().size())
                ...
                if(student == null)
                    ...
                    AppView.outputLine("! 입력된 순서[" +  order + "]에서 삭제된 학생의 성적은 (" + student.score() + ")점 입니다." );
            }
        }
    }
    ...
    private void removeFirst(){
        ...
        if(this.list().isEmpty())
            ...
            if(student == null)
                AppView.outputLine("! [맨 앞]에서 학생을 삭제하는 작업을 실패하였습니다." );
            else
                AppView.outputLine("! [맨 앞]에서 삭제된 학생의 성적은 (" + student.score() + ")점 입니다." );
            }
        }
    ...
    private void removeLast(){
        ...
        if(this.list().isEmpty())
            ...
            if(student == null)
                AppView.outputLine("! [맨 뒤]에서 학생을 삭제하는 작업을 실패하였습니다." );
            else
                AppView.outputLine("! [맨 뒤]에서 삭제된 학생의 성적은 (" + student.score() + ")점 입니다." );
        }
    }
    ...
    private void removeAny(){
        ...
        if(this.list().isEmpty())
            ...
            if(student == null)
                ...
                AppView.outputLine("! 삭제된 학생의 성적은 (" + student.score() + ")점 입니다." );
        }
    }
        ...
        if(this.list().isEmpty())
            ...
            if(order < 0 || order >= this.list().size())
                ...
                if(this.list().replaceAt(new Student(score), order))
                    AppView.outputLine("! 입력된 순서[" +  order + "]의 학생의 점수가 (" + score + ")로 바뀌었습니다." );
                else
                    AppView.outputLine("! 입력된 순서[" +  order + "]에 학생의 점수를 바꾸는 작업을 실패하였습니다." );
            }
        }
    }
