...
public class Calculator {
    ...
    private Stack<Character> _operatorStack; //생성자
    private PostfixCalculator _postfixCalculator; //생성자
    private String _infixExpression;
    private String _postfixExpression;

    //<GETTER SETTER START>
    private String infixExpression() { return this._infixExpression; }
    private void setInfixExpression(String newInfixExpression) { this._infixExpression = newInfixExpression; }
    private String postfixExpression() { return this._postfixExpression; }
    private void setPostfixExpression(String newPostfixExpression) { this._postfixExpression = newPostfixExpression; }
    private PostfixCalculator postfixCalculator() { return this._postfixCalculator; }
    private void setPostfixCalculator(PostfixCalculator newPostfixCalculator) { this._postfixCalculator = newPostfixCalculator; }
    private Stack<Character> operatorStack() { return this._operatorStack; }
    private void setOperatorStack(Stack<Character> newOperatorStack) { this._operatorStack = newOperatorStack; }
    //<GETTER SETTER END>
    ...
    private void showOperatorStack(String anOperationLabel) {
        ...
        for(int i = 0; i < this.operatorStack().size(); i++)
            AppView.outputDebugMessage(
                    ((ArrayList<Character>) this.operatorStack()).elementAt(i).charValue() + " "
            );
    ...
    private int inComingPrecedence(Character aToken) {
        ...
        switch (aToken.charValue()) {
            case '(': return 20;//inStack에서 0
            case ')': return 19;
            case '^': return 17;//inStack에서 16
            case '*': return 13;
            case '/': return 13;
            case '%': return 13;
            case '+': return 12;
    ...
    private int inStackPrecedence(Character aToken) {
        ...
        //'(' 위에 쌓임 = 가장 낮은 우선순위, ')' 를 만나면, 그 아예 우선순위를 모두 pop()
        //오른쪽 우선 결합법칙
        //'^' 입력토큰은 17, 스택안은 16을 가짐, 스택에 삽입되기 위해서
        switch (aToken.charValue()) {
            case '(': return 0; //inComing에서 20
            case ')': return 19;
            case '^': return 16;//inComing에서 17
            case '*': return 13;
            case '/': return 13;
            case '%': return 13;
            case '+': return 12;
