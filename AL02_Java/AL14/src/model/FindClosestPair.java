...
public class FindClosestPair {
    ...
    public void setPointSet(PointSet newPointSet) {
    ...
    public PairOfPoints solveByDivideAndConquer(PointSet pointSetForSolve) {
        if (pointSetForSolve == null || pointSetForSolve.size() <= 1) return null;
    ...
    private PairOfPoints solveRecursively(ReferenceList Px, ReferenceList Py) {
        ...
        if (closestPairFromLeft == null) closestPair = closestPairFromRight;
        else if (closestPairFromRight == null) closestPair = closestPairFromLeft;
        else if (closestPairFromLeft.distance() < closestPairFromRight.distance()) closestPair = closestPairFromLeft;
        else closestPair = closestPairFromRight;
    ...
    private SeparatedPair separateReferenceList(ReferenceList referenceList, int separationLine) {
        ...
        for (int index = 0; index < sizeOfReferenceList; index++) {
            ...
            if (pointReferencedByIndex_i.x < separationX) separatedPair._leftList.add(pointReference);
            else if (pointReferencedByIndex_i.x > separationX) separatedPair._rightList.add(pointReference);
            else {
                if (pointReference < separationLine) separatedPair._leftList.add(pointReference);
                else separatedPair._rightList.add(pointReference);
            }
        }
    ...
    private PairOfPoints closestPairFromDeltaBand(ReferenceList Py, Integer separationLine, long delta) {
        ...
        if (Sy_size > 1) {
            ...
            for (hereIndex = 1; hereIndex < Sy_size - 1; hereIndex++) {
                PairOfPoints closestPairToNeighbors =
                        this.closestPairFromHereToNeighbors(Sy, hereIndex);
                if (closestPairToNeighbors.distance() < closestPair.distance()) closestPair = closestPairToNeighbors;
            }
        }
        return closestPair;
    }
    ...
    private PairOfPoints closestPairFromHereToNeighbors(ReferenceList Sy, int hereIndex) {
        ...
        for (int neighborIndex = (hereIndex + 2);
