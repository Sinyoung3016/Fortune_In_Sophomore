...
public abstract class ReferenceListOrderedByCoordinate extends ReferenceList {
    ...
    private int partition ( int left, int right ) {
        ...
        int toLeft = right + 1 ;
        do {
            do { toRight ++ ; } while ( this.compareCoordinate (toRight, pivotIndex) < 0 ) ;
            do { toLeft-- ; } while ( this.compareCoordinate (toLeft, pivotIndex) > 0 ) ;
            if ( toRight < toLeft ) {
    ...
    private void sortByCoordinateRecursively(int left, int right ) {
        if ( left < right ) {
            int mid = this.partition (left, right) ;
            this.sortByCoordinateRecursively(left, mid-1) ;
            this.sortByCoordinateRecursively(mid+1, right) ;
        }
    }
    private void sortByCoordinate ()
    {
        if ( this.size() >= 2 ) {
            ...
            this.sortByCoordinateRecursively(0, this.size()-2) ;
        }
    }
