 ...
 */
int tmin(void) {
    return 1 << 31; //0x00000001 << 31 = 0x800000000 
}
/*
...
unsigned float_abs(unsigned uf) {
    int eraseSignbitfromuf = uf << 1;
    //overflow
    if (0xff000002 <= eraseSignbitfromuf && eraseSignbitfromuf <= 0xfffffffe) return uf; 
    uf = uf & 0x7fffffff; //changeSignBit0 
    return uf;
}
/*
...
int addOK(int x, int y) {
    int msbFromX = ((x >> 31) & 0x1);
    int msbFromY = ((y >> 31) & 0x1);
    int toMakeCarry = ~(0x8 << 28);
    int carry = (((x & toMakeCarry) + (y & toMakeCarry)) >> 31) & 0x1;
    return !(msbFromX|carry) + (msbFromY&carry) + (msbFromX&(!msbFromY));
}   
/*
...
int replaceByte(int x, int n, int c) {
    int shift = (n << 3);
    int MakeCToFit = c << shift;
    int shiftToMakePlaceEmpty = ~(0xff << (n << 3));
    int ChangeXtoReplace = x & shiftToMakePlaceEmpty;
    return MakeCToFit + ChangeXtoReplace;
}
/*
